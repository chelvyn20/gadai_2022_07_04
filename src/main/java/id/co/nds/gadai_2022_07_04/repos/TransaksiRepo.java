...
public interface TransaksiRepo
	...
	@Query(value = "SELECT * fROM tx_transaksi_cicilan_tetap AS t "
			+ " JOIN ms_product AS p ON  p.product_id = t.product_id "
			+ " JOIN ms_customer AS c ON c.customer_id = t.customer_id ", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM tx_transaksi_cicilan_tetap WHERE no_transaksi = ?1 ", nativeQuery = true)
	TransaksiCicilanTetapEntity findDetailByNoTransaksi(String noTransaksi);

	@Modifying
	@Query(value = "UPDATE tx_transaksi_cicilan_tetap SET status_tx = 'LUNAS' "+
	" WHERE no_transaksi =?1 ",nativeQuery = true)
	void updateStatusByTransaksi(String noTransaksi);
}
